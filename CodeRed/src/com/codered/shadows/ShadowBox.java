...
public class ShadowBox {
	...
	protected void update()
	{
		...
		Vector3f forwardVector = new Vector3f(rotation.transformN(FORWARD));

		Vector3f toFar = forwardVector.mulN(SHADOW_DISTANCE);
		Vector3f toNear = forwardVector.mulN(near);
		
		Vector3f centerNear = toNear.addN(cam.getTotalPos());
		Vector3f centerFar = toFar.addN(cam.getTotalPos());
	...
	protected Vector3f getCenter()
	{
		...
		Matrix4f invertedLight = lightViewMatrix.inversed();
		return invertedLight.transform(cen);
	}
	...
	{
		Vector3f upVector = new Vector3f(rotation.transformN(UP));
		Vector3f rightVector = forwardVector.cross(upVector);
		Vector3f downVector = new Vector3f(-upVector.x, -upVector.y, -upVector.z);
		Vector3f leftVector = new Vector3f(-rightVector.x, -rightVector.y, -rightVector.z);
		
		
		Vector3f farTop = centerFar.add(upVector.mulN(farHeight));
		Vector3f farBottom = centerFar.add(downVector.mulN(farHeight));
		Vector3f nearTop = centerNear.add(upVector.mulN(nearHeight));
		Vector3f nearBottom = centerNear.add(downVector.mulN(nearHeight));
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction,
			float width)
	{
		Vector3f point = startPoint.addN(new Vector3f(direction.x * width, direction.y * width, direction.z * width));
	...
	private Matrix4f calculateCameraRotationMatrix()
	{
		...
		rotation.rotate(new Vector3f(0, 1, 0), (float)-cam.getTransform().getRotation().getEulerYaw());
		rotation.rotate(new Vector3f(1, 0, 0), (float)-cam.getTransform().getRotation().getEulerPitch());
	...
	{
		return (float) this.context.getWidth() / (float) this.context.getHeight();
	}
